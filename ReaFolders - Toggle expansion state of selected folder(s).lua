-- =====================================================================================================================
-- DESCRIPTION:
--     Toggles the expanded/collapsed state of all folder tracks that are currently selected in the Track Control
--     Panel.  That is, all folder tracks currently found to be expanded get collapsed, and all folder tracks
--     currently found to be collapsed get expanded.  A folder track is defined as a track that has gone
--     through the conversion process described by the [Convert selected track(s) to folders] script.
-- =====================================================================================================================
...
for iSelectedTrack = 1, #selectedTracks do
    ...
    -- If the selected track is a folder track, toggle its expand/collapse state.
    if (IsFolderTrack(selectedTrack)) then
        if (IsCollapsedFolderTrack(selectedTrack)) then
            ExpandFolderTrack(selectedTrack, selectedTrackNumber, projectTrackCount)
        else
            CollapseFolderTrack(selectedTrack, selectedTrackNumber, projectTrackCount)
        end
    end
    DebugMsgL("--------------------------------------------------------------------------------")
end
